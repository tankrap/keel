...
# Free: do two report.json runs agree within statistical noise? (manifest-gated). No API, no key.
...
**Reproducing the stochastic (LLM) parts.** The manifest makes the *inputs* verifiable, but the LLM calls are stochastic — the same inputs won't reproduce byte-for-byte. So "reproducible" for the measured rates means *no statistically significant difference* between two runs, not equal counts. `python3 run_suite.py --compare run_a.json run_b.json` decides that: per benchmark × condition it runs a two-proportion z-test at 95% and passes only if every condition agrees within noise. It is **manifest-gated** — if the two runs' `manifest_sha256` differ, their inputs differed, so it reports a hard mismatch instead of a noise verdict (a differing `environment.keel` commit is surfaced as a note). This is how you confirm a re-run of a published result reproduces: same manifest SHA, and `--compare` PASS.
...
| `bench_compare.py` | reproduce-within-noise check: two-proportion z-test over two reports, manifest-gated (no API) |
